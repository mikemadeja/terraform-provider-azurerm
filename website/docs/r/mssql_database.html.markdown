...
* `storage_account_type` - (Optional) Specifies the storage account type used to store backups for this database. Possible values are `Geo`, `Local`, `Zone` and `GeoZone`. The default value is `Geo`.
